...
void TMS(void *parameter) {
  ...
  while (1)
  {
    ...
    for (int i = 0; i < car_size; i++)
    {
      if (cars[i][1] != 0 && cars[i][8] == 0) //Arrival time = 0? We don't care about those. We only give the following points to new arriving cars.
      {
    ...
    for (int i = 0; i < car_size; i++) { //Block every illegal crossing
      if (!scheduled[i] && (tms[cars[queue[i]][2]][cars[queue[i]][3]] == 1 || tms[cars[queue[i]][2]][cars[queue[i]][3]] == 1) && cars[queue[i]][1] != 0)
      {
        scheduled[i] = true;
        ...
        tms[cars[queue[i]][2]][cars[queue[i]][3]] = 1;
    ...
    for (int i = 0; i < car_size; i++) { //reset blockades
      if (scheduled[i] && cars[i][7] == 3) {
        scheduled[i] = false;
        xSemaphoreTake(car_mutex, portMAX_DELAY);
        cars[i][7] = 0; //reset. I don't care much at this point. The car is already dead.
        xSemaphoreGive(car_mutex);
    ...
    if (drive)
    {
      switch (target) {
        ...
        case 1: //to checkpoint 2 -> drive straight forward, then change direction
          ...
          {
            target = 2;
            xSemaphoreTake(car_mutex, portMAX_DELAY); //signalize the tms that it shouldn't care about this car anymore
            cars[car[0]][1] = 0;
            xSemaphoreGive(car_mutex);
          }
          break;
